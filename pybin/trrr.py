...
# Generally, put more important ones last for precedence reasons. Assumes
# Python 3.7+ or CPython 3.6+ for ordered dictionary reasons.
...
def sub_flag(alph, flag, repl):
    ...
    new_flags = flags.replace(flag, repl)
    if not new_flags:
        return name
    return "{}_{}".format(name, new_flags)
...
auxiliary["letters"] = "".join(alphabets[a] for a in alphabets
        if has_flag(a, "u") or has_flag(a, "l"))
...
def trrr(from_spec, to_spec, from_spec_sub="", to_spec_sub="",
    ...
    if dryrun:
        ...
        if unbuffered:
            out_file.reconfigure(line_buffering=True)
        for line in in_file:

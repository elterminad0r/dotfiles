"""
Provides the Matrix class. Copied from my lsystems repository because I think
that probably it's a good idea to keep a central matrix class, which isn't a
dependency of sympy. Numpy matrices are obviously nowhere near abstract enough
for my lofty standards.

This is now written in Python 3, so not compatible with Processing. If matrices
needed for processing, probably best to just copy from lsystems again.
"""

# TODO: use the Strassen algorithm for multiplication
# TODO: Implement inversion with Gauss-Jordan and determinants and so on and so
#       forth.
...
class Matrix(object):
    ...
    def identity(cls, n):
        ...
        """
        return cls([[int(x == y) for x in range(n)] for y in range(n)])
    ...
    def spaced_str(self, n=1):
        ...
        if len(self.array) == 0:
            ...
            delims = chain(["/\\"],
                           ...
                           [r"\/"])
        widths = [max(len(str(i)) for i in col) for col in zip(*self.array)]
        return "\n".join("{}{}{}".format(ldelim,
                                         " ".join(starmap("{!s:>{}}".format,
                                                          zip(row, widths))),
                                         rdelim)
                for row, (ldelim, rdelim) in
                    intersperse_it(zip(self.array, delims),
                                   (([""] * len(self.array[0]), "||"),) * n))
